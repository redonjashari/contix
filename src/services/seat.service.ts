...
// import QRCode from 'qrcode'; // not used here, but keep if needed elsewhere

// ----- Types -----
export interface CreateHoldRequest {
  userId: string;
  eventId: string;
  seatCodes: string[]; // Format: "section-row-number" e.g., "A-12-5"
  ttlSeconds?: number;
}
...
export class SeatService {
   ...
   */
  async createHold(request: CreateHoldRequest): Promise<HoldResult> {
    ...
    return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const seats = await tx.$queryRaw<Array<any>>`
        SELECT id, section, row, number, price, status
        FROM seats
        WHERE event_id = ${eventId}
        AND (section, row, number) IN (${Prisma.join(
          seatIdentifiers.map(s => Prisma.sql`(${s.section}, ${s.row}, ${s.number})`)
        )})
        FOR UPDATE
      `;
      ...
      if (unavailableSeats.length > 0) {
        throw new Error(
          `Seats already taken: ${unavailableSeats
            .map((s: any) => `${s.section}-${s.row}-${s.number}`)
            .join(', ')}`
        );
      }

      const holdRecords = seats.map((seat: any) => ({
        id: uuidv4(),
        userId,
        eventId,
        seatId: seat.id,
      ...
      await tx.seat.updateMany({
        where: { id: { in: seats.map((s: any) => s.id) } },
        data: { status: 'HELD' },
      });
   ...
   */
  async bulkCreateSeats(eventId: string, seats: BulkSeatInput[]) {
    if (!eventId) throw new Error('eventId required');
    if (!Array.isArray(seats) || seats.length === 0) throw new Error('seats must be a non-empty array');
   ...
   */
  async validateHold(holdToken: string, userId: string) {
  ...
  async getEventSeats(eventId: string) {
    ...
    const seatMap = seats.reduce((acc: Record<string, HeldSeat[]>, seat: any) => {
