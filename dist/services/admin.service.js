...
export const AdminService = {
    ...
    async updateEvent(eventId, updates) {
        ...
        if (updates.startTime)
            dataToUpdate.startAt = new Date(updates.startTime);
        if (updates.endTime)
            dataToUpdate.endAt = new Date(updates.endTime);
        ...
        if (updates.genre !== undefined)
            dataToUpdate.genre = updates.genre;
        if (updates.posterPath !== undefined)
            dataToUpdate.posterPath = updates.posterPath;
     ...
     */
    async createSeats(eventId, seats) {
    ...
    async getSalesReport(start, end) {
        ...
        const byDay = new Map(); // date string -> total cents
        ...
        const venueIds = Array.from(new Set(Array.from(byVenue.keys()).filter((id) => !!id)));
        if (venueIds.length > 0) {
            ...
            for (const vid of venueIds) {
                ...
                if (entry)
                    entry.name = nameMap.get(vid) ?? null;
            }
        }
