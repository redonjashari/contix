...
export class PaymentService {
     ...
     */
    async createPaymentIntent(holdToken, userId) {
        // Validate hold
        ...
        const totalAmount = holds.reduce((sum, h) => sum + Number(h.seat.price), 0);
    ...
    async finalizeOrder(orderId) {
        return await prisma.$transaction(async (tx) => {
            ...
            // Generate tickets
            const tickets = await Promise.all(holds.map(hold => ticketService.generateTicket(tx, orderId, hold.seatId)));
            return {
                orderId,
                tickets,
            };
